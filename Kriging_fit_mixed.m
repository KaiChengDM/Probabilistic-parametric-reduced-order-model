function model = Kriging_fit_mixed(inputpar,hyperpar)   

% Training a Kriging model with mixed kernel
% All inputs share the same hyper-parameter
...
for kk = 1: num                                  % Multistart for hype-parameter optimization
 ...
 theta = rand(1,2).*(ub-lb) + lb;
 %theta = 10.^(rand(1,2).*(log_ub-log_lb)+log_lb);
 ...
 switch opt_algorithm   
   ...
   case 'Fmincon'
        ...
        [t_opt(kk,:),fmin(kk)] = fmincon(@(x)Kriging_likelihood(x),theta,[],[],[],[],lb,ub,[],options);
   case 'GA'
        ...
        [t_opt(kk,:),fmin(kk)] = ga(@(x)Kriging_likelihood(x),size(theta,2),[],[],[],[],lb,ub,[],gaoptions);% Genetic algorithm
   case 'CMAES'
        opts.LBounds = lb; opts.UBounds = ub; 
        [t_opt(kk,:),fmin(kk)] = Cmaes(@(x)Kriging_likelihood(x),theta,[],opts);
  end

end

 [value, ind]     = min(fmin);
 model.theta      = t_opt(ind,1);
 model.weight     = t_opt(ind,end);
 model.likelihood = value;
 
 corrmat  = feval(corr_fun,u,model.theta,dim,'off');
 corrmat =  (1-model.weight)*corrmat + model.weight*triu(u*u') + 10^(-12)*eye(m);  % mixed kernel

 [upper_mat rd]  = chol(corrmat);        
 model.upper_mat = upper_mat;
 model.corrmat   = corrmat;
 sigma2 = sum((upper_mat'\y).^2)/m;
 model.sigma2 = sigma2;
...
function Likelihood = Kriging_likelihood(theta) 
   ...
   corrmat  = feval(corr_fun,u,theta(1),dim,'off');

   corrmat =  (1-theta(end))*corrmat + theta(end)*triu(u*u') + 10^(-12)*eye(m);  % mixed kernel

   [upper_mat rd] = chol(corrmat);
   
   sigma2 = sum((upper_mat'\y).^2)/m;

   Likelihood = m*log(sigma2)+2*sum(log(diag(upper_mat)));
...
if  ~isinf(f)
  % Iterate
  p  =  length(t);
  if  p <  2,  kmax  =  5; else,  kmax  =  10; end
  for  k  =  1 : kmax
     th  =  t;
