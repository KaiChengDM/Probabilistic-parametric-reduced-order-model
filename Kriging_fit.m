...
for kk = 1: num                                  % Multistart for hype-parameter optimization
 ...
 corrmat =  (1-model.weight)*corrmat + model.weight*triu(u*u') + 10^(-12)*eye(mn);  % mixed kernel

 [upper_mat rd]  = chol(corrmat);        
 model.upper_mat = upper_mat;
 model.corrmat   = corrmat;
 sigma2          = sum((upper_mat'\y).^2)/mn;
 model.sigma2    = sigma2;
...
function Likelihood = Kriging_likelihood(theta) 
   ...
   corrmat =  (1-theta(end))*corrmat + theta(end)*triu(u*u') + 10^(-12)*eye(mn);  % mixed kernel
