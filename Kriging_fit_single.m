function model = Kriging_fit_single(inputpar,hyperpar)   

% Training a Kriging model with Gaussian kernel
...
for kk = 1: num    % Multistart for hype-parameter optimization
 ...
 theta = rand.*(ub-lb) + lb;
 %theta = 10.^(rand.*(log_ub-log_lb)+log_lb);
 ...
 beta0  = 0;  
 sigma2 = sum((upper_mat'\(y-beta0*f)).^2)/m;
 model.beta0  = beta0;
 model.sigma2 = sigma2;
 model.f  = f;
...
if  ~isinf(f)
  % Iterate
  p  =  length(t);
  if  p <=  2,  kmax  =  5; else,  kmax  =  min(p,10); end
  for  k  =  1 : kmax
    th  =  t;
