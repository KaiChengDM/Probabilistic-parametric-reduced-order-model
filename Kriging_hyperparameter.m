function model = Kriging_hyperparameter(inputpar,hyperpar)
...
model.dim         = n;
model.std_y       = inputpar.std_y ;
model.mu_y        = inputpar.mu_y ;
...
function Likelihood = Kriging_likelihood(theta) 
   ...
   corrmat = corrmat + eye(m).*10^-12;

   for i = 1:nt

       cov = corrmat + diag(var(:,i));
       [upper_mat rd] = chol(cov);
      
       likelihood(i) = sum((upper_mat'\y(:,i)).^2) + 2*sum(log(diag(upper_mat)));
